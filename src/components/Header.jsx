...
export default function Header() {
    ...
    return (
        <header className="fixed top-0 left-0 w-full bg-white/80 backdrop-blur-xl shadow-sm z-50 border-b border-purple-100">
            <div className="max-w-7xl mx-auto flex justify-between items-center py-4 px-6">
                ...
                <Link to="/" className="flex items-center space-x-3 group">
                    ...
                    <div className="hidden sm:block">
                        <div className="text-xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-[#8a48e7] to-[#5c21d2]">
                            Cochin Connect
                        </div>
                    </div>
                </Link>
                ...
                <nav className="hidden md:flex items-center space-x-1">
                    {navLinks.map((link) => (
                        <Link
                            key={link.to}
                            to={link.to}
                            className="relative px-4 py-2 text-gray-700 font-medium rounded-lg hover:text-[#5c21d2] transition-all duration-300 group"
                        >
